...
## Clone and build libosmocore from GitHub
...
/* Basic FreeRTOS configuration for osmocom-bb-freertos build */
...
/* Network related settings for FreeRTOS+TCP */
...
RUN cd "$LIBOSMOCORE_DIR" && \
    # Inject a version so pkg-config reports a non-UNKNOWN version (branch lacks tags)
    echo '1.9.0-freertos' > .tarball-version && \
    # Build libosmocore with FreeRTOS adaptations and enable VTY
    autoreconf -fi && \
    ...
    make -j"$(nproc)" && \
    make install && \
    # Ensure no external talloc dependency leaks into pkg-config metadata
...
## The source will be bind-mounted by docker compose
WORKDIR /workspace
